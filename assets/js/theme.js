            ...
            this._algoliaIndex
              ...
              .then(({ hits }) => {
                ...
                hits.forEach((hit) => {
                  ...
                  // Preferimos el título destacado si está disponible
                  const title = hit._highlightResult?.title?.value || hit.title;
                  
                  // Extraemos el snippet del contenido
                  const content = hit._snippetResult?.content?.value || 
                                  (hit.content ? hit.content.substring(0, snippetLength) + '...' : '');
                  
                  // Creamos el resultado
                  results[url] = {
                    uri: url,
                    title: title,
                    date: hit.date || '', // Campo opcional
                    context: content
                  };
                });
